...
public class SpringControllerHttpVerbsMethodsEntityRule extends BaseTreeVisitor implements JavaFileScanner {
  ...
  public void visitMethod(MethodTree tree) {
    ...
    boolean isRestControllerContext = parentClassOwner.isAnnotatedWith("org.springframework.web.bind.annotation.RestController");
    
    if ((isControllerContext || isRestControllerContext)
      && SpringControllerHttpMethods.exists(methodSymbol.metadata())) {
      ...
      for (VariableTree param : tree.parameters()) {
        ...
        if (typeOfParam.symbolType().symbol().metadata().isAnnotatedWith("javax.persistence.Entity")) {
          context.reportIssue(this, typeOfParam, String.format("You must avoid connect request parameters with your model", typeOfParam.symbolType().name()));
        }
      }
